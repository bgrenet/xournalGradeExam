...
11. (optional) On the preamble (empty pages before the first exam) you can also add a text area starting with `*settings:` followed by a new line and a list of options (one per line) like `my option=my param` to **customize various aspects of this program**. You can copy to your clipboard all available settings with their default parameter by clicking on `Copy all settings`. More precisely we provide:
    - `removeNoGradeCells` (either `false` (default) or `true`) such that, if it is set to `removeNoGradeCells=true`, will export in the CSV any cell with no grade as an empty cell instead of writing a more visible `NO GRADE` text.
    - `keepStudentsNoExam` (either `true` (default) or `false`): if set to `false`, removes in the exported CSV the users found in the reference list that have no exam.
    - `CsvAddStats` (either `true` (default) or `false`): if set to `false`, we removes the statistics in the generated CSV, like TOTAL grades etc.
    - `CsvSymbolSum`: set it to the `SUM` translation in your language (e.g. `CsvSymbolSum=SOMME`) so that your spreadsheet program can import the CSV file properly. We try to automatically guess it based on the `LANG` environment variable (not available on windows by default but you can add it if you want).
...
13. **Import this CSV** in your favorite spreadsheet editor (Excel, Libre Office Calc…). At least in Libre Office Calc, **make sure to select the `English` locale** when importing the CSV (otherwise it might be confused by english-named formulas and by grades with dots like `2.5` if your default locale expects comma-separated numbers) and click on a field named like `Evaluate formulas` since in percentage mode we use a formula to allow you to later change the scale for each question in excel. By default, we also include some "statistics", notably the sum of all grades per student to provide the final grade. Since spreadsheet programs (including Libre Office) use different formula based on the language of the user like `SUM` in english, `SOMME` in french etc, we try to guess it (based on the `LANG` environment variable used in Linux, if you have Windows feel free to add a detection mechanism since I don't have a windows with me to test). Yet, you can still customize it by setting the `CsvSymbolSum=YOURSUM` setting (see above to configure them).
